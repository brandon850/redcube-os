// Lightweight site crawler — ported from the RedCube SEO platform (Express → TS).
// Server-side only (cheerio + global fetch). Do NOT import from client code.
import * as cheerio from 'cheerio'
import type { AuxFiles, CrawledPage, PageMetrics } from './types'
...
const NON_PAGE_EXTENSIONS = new Set([
  ...
  'js', 'css', 'json', 'xml', 'rss', 'atom', 'map', 'ts', 'jsx', 'tsx',
])
...
async function fetchPage(url: string): Promise<FetchResult> {
  ...
  try {
    ...
    return { html: '', status: 0, finalUrl: url, ok: false, error: e instanceof Error ? e.message : 'fetch failed' }
  } finally {
    clearTimeout(timer)
  }
}
...
function extractInternalLinks($: Api, pageUrl: string, origin: string): Set<string> {
  ...
  $('a[href]').each((_, el) => {
    ...
    if (normalized && normalized.startsWith(origin) && isHtmlPage(normalized)) links.add(normalized)
  })
  return links
}

function extractPageMetrics($: Api, html: string, url: string, origin: string): PageMetrics {
  ...
  return {
    ...
    internalLinks: $('a[href]').filter((_, el) => {
      try { return new URL($(el).attr('href') || '', url).origin === origin } catch { return false }
    }).length,
    externalLinks: $('a[href]').filter((_, el) => {
      try { return new URL($(el).attr('href') || '', url).origin !== origin } catch { return false }
    }).length,
    ...
    hasNoindex: robots.toLowerCase().includes('noindex'),
  }
}

export async function crawlSite(startUrl: string, maxPages = MAX_PAGES): Promise<CrawledPage[]> {
  ...
  while (queue.length > 0 && pages.length < maxPages) {
    ...
    const page: CrawledPage = { url, finalUrl, status, ok, error: error ?? null, metrics: null }
...
export async function checkAuxFiles(baseUrl: string): Promise<AuxFiles> {
