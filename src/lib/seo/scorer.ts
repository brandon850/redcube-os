// SEO scorer — ported from the RedCube SEO platform. Six weighted categories,
// plain-English findings, A+–F grade. Server-side only.
import type { AuditResult, AuxFiles, CategoryResult, CrawledPage, Finding, PageMetrics } from './types'

function scoreToStatus(s: number): string {
  return s >= 90 ? 'Great' : s >= 75 ? 'Good' : s >= 60 ? 'Okay' : s >= 40 ? 'Needs Work' : 'Poor'
}

export function scoreSite(pages: CrawledPage[], aux: AuxFiles, startUrl: string): AuditResult {
  const hm: Partial<PageMetrics> = pages[0]?.metrics ?? {}
  const all: PageMetrics[] = pages.map((p) => p.metrics).filter((m): m is PageMetrics => !!m)
  const n = all.length || 1
  const isHttps = startUrl.startsWith('https://')

  // ── FINDABILITY ──
  let findScore = 100
  const findFindings: Finding[] = []
  ...
  const overall = Math.round(
    findScore * 0.25 + speedScore * 0.15 + mobileScore * 0.2 +
    trustScore * 0.2 + contentScore * 0.12 + navScore * 0.08,
  )
